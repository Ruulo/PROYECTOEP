<?php
	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Cell(30);
			$this->Cell(220,10, 'Lista de Vaporizador/Sistema de Gas',0,0,'C');
			$this->Ln();
        ...
        { 
	     ...
	     function viewTable($db)
		 {
          ...
          $stmt = $db->query("SELECT id_v,turno_v,equipo_v,sistema_v,falla_v,refa_v FROM vaporizador INNER JOIN usuario ON vaporizador.usuario_id=usuario.idusuario WHERE idusuario=2 AND DATE_FORMAT(fecha_v, '%Y-%m-%d') = CURDATE()");
          while($data=$stmt->fetch(PDO::FETCH_OBJ)){
		 $this->Cell(18,10,$data->id_v,1,0,'C');
         $this->Cell(20,10,$data->turno_v,1,0,'C');
         $this->Cell(52,10,$data->equipo_v,1,0,'C');
         $this->Cell(60,10,utf8_decode($data->sistema_v),1,0,'C');
         $this->Cell(60,10,utf8_decode($data->falla_v),1,0,'C');
		 $this->Cell(50,10,utf8_decode($data->refa_v),1,0,'C');
		 $this->Ln();
		 }
	   }		  
	}
	...
	$pdf->headerTable();
	$pdf->viewTable($db);
	$pdf->Output('Vaporizador/Sistema gas.pdf','D');
?>
