<?php
	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Cell(30);
			$this->Cell(140,10,utf8_decode('Reporte por dìa del estado de Telares Circulares'),0,0,'C');
			$this->Ln();
...
$consulta = mysql_query("SELECT turno_tc,id_tc,equipo_tc FROM telar_circular INNER JOIN usuario ON telar_circular.usuario_id=usuario.idusuario WHERE idusuario=4 AND estado_tc='pendiente' AND DATE_FORMAT(fecha_tc, '%Y-%m-%d') = CURDATE()");
...
while($resultado = mysql_fetch_array($consulta)){
 ...
 $pdf->Cell(70,6,$resultado['equipo_tc'],1,0,'C');
  $pdf->Ln(); 
	...
	$pdf->Output('Telares Circulares Pendientes por dìa.pdf','D');
?>
