<?php
	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Cell(30);
			$this->Cell(140,10,utf8_decode('Reporte por dìa del estado de Telares Planos'),0,0,'C');
			$this->Ln();
...
$consulta = mysql_query("SELECT turno_tp,id_tp,telar_tp FROM telar_planos INNER JOIN usuario ON telar_planos.usuario_id=usuario.idusuario WHERE idusuario=4 AND estado_tp='pendiente'  AND DATE_FORMAT(fecha_tp, '%Y-%m-%d') = CURDATE()");
...
while($resultado = mysql_fetch_array($consulta)){
 ...
 $pdf->Cell(70,6,$resultado['telar_tp'],1,0,'C');
  $pdf->Ln(); 
	...
	$pdf->Output('Pendientes por dia.pdf','D');
?>
