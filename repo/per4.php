<?php
	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Ln();
			$this->Cell(246,10,'Realizador por el Supervisor: Jesus.Valero@bagcorp.com',0,0,'C');
			$this->Ln();
        ...
        { 
	     ...
	     function viewTable($db)
		 {
          ...
          $stmt = $db->query("SELECT id_pe, turno_pe,nombre_pe,apellido_pe,fecha_pe,com_pe,(ELT(WEEKDAY(fecha_pe) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo')) AS DIA_SEMANA FROM personal INNER JOIN usuario ON personal.usuario_id=usuario.idusuario WHERE idusuario=4 AND DATE_FORMAT(fecha_pe, '%Y-%m-%d') = CURDATE()");
