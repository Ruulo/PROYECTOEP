<?php
	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Cell(30);
			$this->Cell(140,10,utf8_decode('Reporte por mes del estado de Calentadores'),0,0,'C');
			$this->Ln();
...
$consulta = mysql_query("SELECT turno_c,id_c,equipo_c FROM calentadores INNER JOIN usuario ON calentadores.usuario_id=usuario.idusuario WHERE MONTH(fecha_c) = MONTH(CURDATE()) AND estado_c='pendiente'");
...
while($resultado = mysql_fetch_array($consulta)){
 ...
 $pdf->Cell(70,6,$resultado['equipo_c'],1,0,'C');
  $pdf->Ln(); 
	...
	$pdf->Output('Calentadores Pendientes por mes.pdf','D');
?>
