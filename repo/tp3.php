<?php
	...
	class PDF extends FPDF
	{
		function Header()
		{
			...
			$this->Cell(30);
			$this->Cell(220,10, 'Lista de Estado de Telares Planos',0,0,'C');
			$this->Ln();
			...
			$this->Cell(2,10,'Hoy:'.date('d/m/Y'),0,0,'C');
			$this->Ln();
        ...
        { 
		 ...
		 {
		  $this->SetFont('Arial','','8');
          $stmt = $db->query("SELECT id_tp,turno_tp,telar_tp,estilo_tp,sistema_tp,falla_tp,refa_tp,tipotelar_tp FROM telar_planos INNER JOIN usuario ON telar_planos.usuario_id=usuario.idusuario WHERE idusuario=3  AND estado_tp='pendiente' AND DATE_FORMAT(fecha_tp, '%Y-%m-%d') = CURDATE()");
	...
	$pdf->headerTable();
	$pdf->viewTable($db);
	$pdf->Output('Telares Planos.pdf','D');
?>
