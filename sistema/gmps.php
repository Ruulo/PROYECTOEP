...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'TRANSPORTE DE PLEGADIRES ESTADO PENDIENTE POR MES'
        },
        tooltip: {
        ...
        series: [{
			...
			<?php
			$sql_re=mysqli_query($conection,"select DISTINCT id_p from plegadores where MONTH(fecha_p) = MONTH(CURDATE()) AND estado_p='Pendiente'");
			while($res=mysqli_fetch_array($sql_re)){
			?>
			
                ['<?php echo $res['id_p']; ?>', <?php echo $res['id_p'] ?>],
