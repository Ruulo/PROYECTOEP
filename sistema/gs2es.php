...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text:'TELAR PROCESO FALLA "EN ESPERA DE TRAMA" POR SEMANA'
        },
        subtitle: {
            text: ''
        },
        ...
        xAxis: {
            categories: [
			<?php
$sql_re=mysqli_query($conection,"SELECT  count(*) AS ESTADO, DATE_ADD(fecha_pp, INTERVAL 8-DAYOFWEEK(fecha_pp) DAY) SEMANA FROM telar_proceso where falla_pp='Fuera de programa' AND DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()");
...
?>
        ...
        series: [{
            name: 'EN ESPERA DE TRAMA ',
            data: [
			
			<?php
$sql_re=mysqli_query($conection,"SELECT  count(*) AS ESTADO, DATE_ADD(fecha_pp, INTERVAL 8-DAYOFWEEK(fecha_pp) DAY) SEMANA FROM telar_proceso where falla_pp='En espera de trama' AND DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()");
while($res=mysqli_fetch_array($sql_re)){			
?>			
...
<a href="gmes.php" class="btn_new" > Siguiente <span class="fi fi-arrow-right"></a></span></center>
