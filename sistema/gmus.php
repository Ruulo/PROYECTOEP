...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'URDIDOR ESTADO PENDIENTE POR MES'
        },
        tooltip: {
        ...
        series: [{
			...
			<?php
			$sql_re=mysqli_query($conection,"select DISTINCT id_u from urdidor where MONTH(fecha_u) = MONTH(CURDATE()) AND estado_u='Pendiente'");
			while($res=mysqli_fetch_array($sql_re)){
			?>
			
                ['<?php echo $res['id_u']; ?>', <?php echo $res['id_u'] ?>],
