...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'CALENTADORES ESTADO PENDIENTE POR MES'
        },
        tooltip: {
        ...
        series: [{
			...
			<?php
			$sql_re=mysqli_query($conection,"select DISTINCT equipo_c from calentadores where MONTH(fecha_c) = MONTH(CURDATE()) AND estado_c='Pendiente'");
			while($res=mysqli_fetch_array($sql_re)){
			?>
			
                ['<?php echo $res['equipo_c']; ?>', <?php echo $res['equipo_c'] ?>],
