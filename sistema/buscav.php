...
{
	header("location:vaporizador1.php");
}
...
$buscar="";
if($fecha_de > $fecha_a){
	header("local:vaporizador1.php");
}else if($fecha_de == $fecha_a){
	
	$where="fecha_v like '$fecha_de%'";
	$buscar="fecha_de=$fecha_de&$fecha_a=$fecha_a";
}else{
	$where="fecha_v BETWEEN '$fecha_de' AND '$fecha_a'";
	$buscar="fecha_de=$fecha_de&$fecha_a=$fecha_a";
      }
   }
?>
			  ...
			  <th>REFACCIÒN</th>
			   ...
			   <th>TIPO DE FALLA</th>
				...
				<th>ACCIONES</th>
				 	...
				 	<!--buscador-->
<form action="buscav.php" method="get">
<div class="right">
...
<br>
<!--<a href="nv.php" class="btn_new"><span class="icon-plus"> Nuevo Registro</a></span>--->
 	   <button class="btn btn-secondary">
                     <a href="../registros/vaporizador.php" class=""><span class="icon-eye">Todos los registros</span> 
</button>  </a>  <br>

        <!--termina el buscador-->
	 <br>
	 <form action="buscav.php"method="get"> De:
...
<!--mostrar datos en tabla-->
         <?php
//paginador 
$sql_registe= mysqli_query($conection,"SELECT COUNT(*) as total FROM vaporizador WHERE $where AND MONTH(fecha_v) = MONTH(CURDATE())");
...
$total_paginas =ceil($total /$por_pagina);
         ...
         $query =mysqli_query($conection, "SELECT * FROM vaporizador WHERE $where AND MONTH(fecha_v) = MONTH(CURDATE()) LIMIT $desde, $por_pagina");
        ...
        'Terminado' => "#E7E0B2"
          ...
          <tr>
            ...
            <td><?php echo $data['fecha_v']; ?></td>
			<td><?php echo $data['equipo_v']; ?></td>
            <td><?php echo $data['sistema_v']; ?></td>
            <td><?php echo $data['falla_v']; ?></td>
			<td><?php echo $data['refa_v']; ?></td>
			 <td bgcolor="<?php $estado_color[$data['estado_v']] ?>"><?php echo $data['estado_v']; ?></td>
			<td><?php echo $data['tipofalla_v']; ?></td>
			<td><?php echo $data['com_v']; ?></td>
            <td>
			 <a href="editev.php?id_v=<?php echo $data['id_v']?>"class="link_edit"><h6>Editar
                <i class="icon-edit"></i></h6>
				</a>
				<a href="eliv.php?id_v=<?php echo $data['id_v']?>" class="eli"><h6>Eliminar
