...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'TELAR JARETA ESTADO PENDIENTE POR MES'
        },
        tooltip: {
        ...
        series: [{
			...
			<?php
			$sql_re=mysqli_query($conection,"select DISTINCT equipo_tj from telar_jareta where MONTH(fecha_tj) = MONTH(CURDATE()) AND estado_tj='Pendiente'");
			while($res=mysqli_fetch_array($sql_re)){
			?>
			
                ['<?php echo $res['equipo_tj']; ?>', <?php echo $res['equipo_tj'] ?>],
...
<a href="a.php" class="btn_new" ><span class="icon-logout"> Salir</a></span>
