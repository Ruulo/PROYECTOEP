...
{
	header("location:calentadores1.php");
}
...
if(!is_numeric($_REQUEST['busqueda'])){
	header("local:calentadores1.pp");
}
$busqueda=strtolower($_REQUEST['busqueda']);
$where="equipo_c=$busqueda";
...
$buscar="";
if($fecha_de > $fecha_a){
	header("local:calentadores1.php");
}else if($fecha_de == $fecha_a){
	
	$where="fecha_c like '$fecha_de%'";
	$buscar="fecha_de=$fecha_de&$fecha_a=$fecha_a";
}else{
	$where="fecha_c BETWEEN '$fecha_de' AND '$fecha_a'";
	$buscar="fecha_de=$fecha_de&$fecha_a=$fecha_a";
      }
   }
?>
...
<!--mostrar datos en tabla-->
          <?php
//paginador 
$sql_registe=mysqli_query($conection,"SELECT COUNT(*) as total FROM calentadores WHERE $where AND MONTH(fecha_c) = MONTH(CURDATE())"); 
...
$query=mysqli_query($conection,"SELECT * FROM calentadores WHERE $where AND MONTH(fecha_c) = MONTH(CURDATE()) LIMIT $desde, $por_pagina");	 
