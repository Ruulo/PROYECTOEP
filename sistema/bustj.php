...
{
	header("location:telar_jareta1.php");
}
...
if(!is_numeric($_REQUEST['busqueda'])){
	header("local:telar_jareta1.pp");
}
$busqueda=strtolower($_REQUEST['busqueda']);
$where="equipo_tj=$busqueda";
...
$buscar="";
if($fecha_de > $fecha_a){
	header("local:telar_jareta1.php");
}else if($fecha_de == $fecha_a){
	
	$where="fecha_tj like '$fecha_de%'";
	$buscar="fecha_de=$fecha_de&$fecha_a=$fecha_a";
}else{
	$where="fecha_tj BETWEEN '$fecha_de' AND '$fecha_a'";
	$buscar="fecha_de=$fecha_de&$fecha_a=$fecha_a";
      }
   }
?>
...
<br>
        ...
        <h1><center><span class="icon-doc-text-inv" style=" font-size:0.9em; color:#000; text-shadow:1px 1px  #4cb6ea;"></span><b> TELARES DE JARETA</h1></center></b>
        <hr>
...
<form action="bustj.php" method="get">
<div class="right">
...
<br>
<!--<a href="ntj.php" class="btn_new"><span class="icon-plus"> Nuevo Registro</a></span>-->
  	   <button class="btn btn-secondary">
             <a href="../registros/telar_jareta.php" class=""><span class="icon-eye">Todos los registros</span> 
</button>  </a> <br>

 <br>
	 <form action="bustj.php"method="get"> De:
...
<!--mostrar datos en tabla-->
    <?php
//paginador 
$sql_registe= mysqli_query($conection,"SELECT COUNT(*) as total FROM telar_jareta WHERE  $where AND MONTH(fecha_tj) = MONTH(CURDATE())");
...
$total_paginas =ceil($total /$por_pagina);
         ...
         $query =mysqli_query($conection, "SELECT * FROM telar_jareta WHERE  $where AND MONTH(fecha_tj) = MONTH(CURDATE()) LIMIT $desde, $por_pagina");
        ...
        'Terminado' => "#E7E0B2"
          ...
          <tr>
            <td><?php echo $data['id_tj']; ?></td>
            <td><?php echo $data['turno_tj']; ?></td>
            <td><?php echo $data['fecha_tj']; ?></td>
			<td><?php echo $data['equipo_tj']; ?></td>
            <td><?php echo $data['sistema_tj']; ?></td>
            <td><?php echo $data['falla_tj']; ?></td>
			<td><?php echo $data['refa_tj']; ?></td>
			<td bgcolor="<?php $estado_color[$data['estado_tj']] ?>"><?php echo $data['estado_tj']; ?></td>
            <td><?php echo $data['tipofalla_tj']; ?></td>
			<td><?php echo $data['com_tj']; ?></td>
            <td>
			 <a href="actj.php?id_tj=<?php echo $data['id_tj']?>" class="link_edit"><h6>Editar
