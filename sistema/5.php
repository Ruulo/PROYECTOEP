...
<body>
 ...
 <center>
  <?php
			...
			$constulta = mysqli_query($conexion, "SELECT nombre from usuario where idusuario=1 ")
				or die ("Error al observar los registros");
				
				
					echo '<table width="174" border="">';
			echo '<tr>';
		    echo '<th1 Jefe="nombre":>REALIZADO POR EL JEFE DEL DEPARTAMENTO: </th>';
	        echo '</tr>';
 ...
 <center>
  <?php
			...
			$constulta = mysqli_query($conexion, "SELECT DISTINCT turno_pp FROM telar_proceso INNER JOIN usuario ON telar_proceso.usuario_id=usuario.idusuario where idusuario=1 AND  DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
<div id="apDiv1">
  ...
  <?php
			...
			$constulta = mysqli_query($conexion, "SELECT id_tp , telar_tp FROM telar_planos INNER JOIN usuario ON telar_planos.usuario_id=usuario.idusuario where estado_tp='Pendiente' AND idusuario=1 AND  DATE_FORMAT(fecha_tp, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
<div id="apDiv4">
 <?php
			...
			$constulta = mysqli_query($conexion, "SELECT id_pp , telar_pp FROM telar_proceso INNER JOIN usuario ON telar_proceso.usuario_id=usuario.idusuario where falla_pp='Por atar' AND idusuario=1 AND  DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
<?php
			...
			$constulta = mysqli_query($conexion, "SELECT id_pp , telar_pp FROM telar_proceso INNER JOIN usuario ON telar_proceso.usuario_id=usuario.idusuario where falla_pp='Cambio de estilo'AND idusuario=1 AND  DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
<?php
			...
			$constulta = mysqli_query($conexion, "SELECT id_pp , telar_pp FROM telar_proceso INNER JOIN usuario ON telar_proceso.usuario_id=usuario.idusuario where falla_pp='Por terminar' AND idusuario=1 AND DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
<?php
			...
			$constulta = mysqli_query($conexion, "SELECT id_pp , telar_pp FROM telar_proceso INNER JOIN usuario ON telar_proceso.usuario_id=usuario.idusuario where falla_pp='Fuera de programa' AND idusuario=1 AND DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
<?php
			...
			$constulta = mysqli_query($conexion, "SELECT id_pp , telar_pp FROM telar_proceso INNER JOIN usuario ON telar_proceso.usuario_id=usuario.idusuario where falla_pp='En espera de trama' AND idusuario=1 AND DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
<center><p>Telares planos</p></center>
	<center><!--primera tabla-->
		<?php
			...
			$constulta = mysqli_query($conexion, "SELECT id_tp, fecha_tp, turno_tp, telar_tp, estilo_tp, sistema_tp, falla_tp, refa_tp, estado_tp,tipofalla_tp, com_tp FROM telar_planos INNER JOIN usuario ON telar_planos.usuario_id=usuario.idusuario WHERE idusuario=1 AND  DATE_FORMAT(fecha_tp, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
  ...
  $estado_color = array(
		...
		<?php
			...
			$constulta = mysqli_query($conexion, "SELECT id_pp,  fecha_pp, telar_pp, estilo_pp, sistema_pp, falla_pp,tipofalla_pp, com_pp FROM telar_proceso INNER JOIN usuario ON telar_proceso.usuario_id=usuario.idusuario WHERE idusuario=1 AND  DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
	...
	<center><p>Telares Circulares</p></center>
		<?php
			...
			$constulta = mysqli_query($conexion, "SELECT id_tc,  fecha_tc, equipo_tc, sistema_tc, falla_tc, refa_tc, estado_tc, tipofalla_tc,com_tc 	FROM telar_circular INNER JOIN usuario ON telar_circular.usuario_id=usuario.idusuario WHERE  idusuario=1 AND  DATE_FORMAT(fecha_tc, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
  ...
  $estado_color = array(
	...
	<center><!--cuarta tabla-->
		...
		<?php
			...
			or die ("Error en la conexion");
				...
				$constulta = mysqli_query($conexion, "SELECT id_tj, fecha_tj, equipo_tj, sistema_tj, falla_tj, refa_tj, estado_tj, tipofalla_tj, com_tj FROM telar_jareta INNER JOIN usuario ON telar_jareta.usuario_id=usuario.idusuario WHERE idusuario=1 AND DATE_FORMAT(fecha_tj, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
	...
	<center><!--quinta tabla-->
		...
		<?php
			...
			$constulta = mysqli_query($conexion, "SELECT id_u, fecha_u, equipo_u, sistema_u,falla_u, refa_u, estado_u, tipofalla_u, com_u FROM urdidor INNER JOIN usuario ON urdidor.usuario_id=usuario.idusuario WHERE idusuario=1 AND DATE_FORMAT(fecha_u, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
plegadores INNER JOIN usuario ON plegadores.usuario_id=usuario.idusuario
 WHERE idusuario=1 AND DATE_FORMAT(fecha_p, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
calentadores INNER JOIN usuario ON calentadores.usuario_id=usuario.idusuario
 WHERE idusuario=1 AND  DATE_FORMAT(fecha_c, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
vaporizador INNER JOIN usuario ON vaporizador.usuario_id=usuario.idusuario WHERE idusuario=1 AND  DATE_FORMAT(fecha_v, '%Y-%m-%d') = CURDATE()")
				or die ("Error al observar los registros");
...
<button type="button" value="Regresar" class="btn btn-secondary tn-lg" id="regresar" onclick="history.back()" >
 <span class="fi fi-arrow-left">SALIR</span></button>
						<br>			
						
<button type="button" value="Imprime esta pagina" onclick="window.print()" class="btn btn-primary tn-lg">
	 <span class="icon-file-pdf">PDF</span></button>
						<br>

<a href="mailto:Jesus.Valero@bagcorp.com; Jose.silva@bagcorp.com;Lucio.Torres@bagcorp.com;Gustavo.OrtegaTello@bagcorp.com; Virgilio.Mendoza@bagcorp.com;Ramiro.Sifuentes@bagcorp.com; Homero.Alvarez@bagcorp.com; Humberto.Deleon@bagcorp.com;Ramon.Acosta@bagcorp.com;Jose.Martinez@bagcorp.com;Armando.Vazquez@bagcorp.com; Roberto.Torres@bagcorp.com?subject=REPORTE MANTENIMIENTO TEJIDO PLANO,CIRCULARES,JARETA Y URDIDO&body=">Enviar por correo</a>
