...
if(empty($busqueda))
{
	header("location:telar_jareta.php");
...
<button type="submit" class="search"><span class="icon-search" ></span>
		</form></button></div><!--se termina el buscador-->
...
<!--mostrar datos en tabla-->
    <?php
//paginador 
$sql_registe= mysqli_query($conection,"SELECT COUNT(*) as total FROM telar_jareta WHERE equipo_tj LIKE '%$busqueda%' AND DATE_FORMAT(fecha_tj, '%Y-%m-%d') = CURDATE()");
...
$total_paginas =ceil($total /$por_pagina);
         ...
         $query = "SELECT * FROM telar_jareta WHERE equipo_tj LIKE '%$busqueda%' AND DATE_FORMAT(fecha_tj, '%Y-%m-%d') = CURDATE()LIMIT $desde, $por_pagina";
