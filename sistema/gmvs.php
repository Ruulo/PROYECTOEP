...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'VAPORIZADOR ESTADO PENDIENTE POR MES'
        },
        tooltip: {
        ...
        series: [{
			...
			<?php
			$sql_re=mysqli_query($conection,"select DISTINCT id_v from vaporizador where MONTH(fecha_v) = MONTH(CURDATE()) AND estado_v='Pendiente'");
			while($res=mysqli_fetch_array($sql_re)){
			?>
			
                ['<?php echo $res['id_v']; ?>', <?php echo $res['id_v'] ?>],
