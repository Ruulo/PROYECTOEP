...
{
	header("location:plegadores1.php");
}
...
$buscar="";
if($fecha_de > $fecha_a){
	header("local:plegadores1.php");
}else if($fecha_de == $fecha_a){
	
	$where="fecha_p like '$fecha_de%'";
	$buscar="fecha_de=$fecha_de&$fecha_a=$fecha_a";
}else{
	$where="fecha_p BETWEEN '$fecha_de' AND '$fecha_a'";
	$buscar="fecha_de=$fecha_de&$fecha_a=$fecha_a";
      }
   }
?>
...
<br>
<br>	   
        <h1><center><span class="icon-doc-text-inv" style=" font-size:0.9em; color:#000; text-shadow:1px 1px  #4cb6ea;"></span><b></span> TRANSPORTE DE PLEGADORES</h1></center></b>
        <hr>
	 ...
	 <div class="table-responsive" style="overflow-x:auto; bgcolor:red">
      <table class="table table-striped">
        <thead>
		 ...
		 <!--buscador-->
<form action="buscple.php" method="get">
<div class="right">
<input type="text" name="busqueda" id="busqueda" placeholder="busca por id.." value="<?php echo $busqueda; ?>">
<button type="submit" class="search"><span class="icon-search" ></span>
</form></button></div>
<!--<a href="np.php" class="btn_new"><span class="icon-plus"> Nuevo Registro</a></span>--->
 	   <button class="btn btn-secondary">
             <a href="../registros/plegadores.php" class=""><span class="icon-eye">Todos los registros</span> 
</button>  </a>  <br>
<!--<a href="graficas17.php" class="btn_r" ><span class="icon-cancel"> Incidencias</a></span><!--botton del pdf-->          
	 <!--termina el buscador-->
        <!--buscador por fechas-->
	 <br>
	 <form action="buscple.php"method="get"> De:
...
<?php
//paginador 
$sql_registe= mysqli_query($conection,"SELECT COUNT(*) as total FROM plegadores WHERE $where AND MONTH(fecha_p) = MONTH(CURDATE())");
...
$total_paginas =ceil($total /$por_pagina);
         ...
         $query =mysqli_query($conection, "SELECT * FROM plegadores WHERE $where AND MONTH(fecha_p) = MONTH(CURDATE()) LIMIT $desde, $por_pagina");
        ...
        'Terminado' => "#E7E0B2"
          ...
          <tr>
			...
			<td><?php echo $data['equipo_p']; ?></td>
            <td><?php echo $data['sistema_p']; ?></td>
            <td><?php echo $data['falla_p']; ?></td>
			<td><?php echo $data['refa_p']; ?></td>
			<td bgcolor="<?php $estado_color[$data['estado_p']] ?>"><?php echo $data['estado_p']; ?></td>
			<td><?php echo $data['tipofalla_p']; ?></td>
			<td><?php echo $data['com_p']; ?></td>
            <td>
			  <a href="acp.php?id_p=<?php echo $data['id_p']?>"class="link_edit"><h6>Editar
                <i class="icon-edit"></i></h6>
				</a>
			  <a href="elip.php?id_p=<?php echo $data['id_p']?>"  class="eli"><h6>Eliminar
