...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'TELAR PROCESO FALLA "POR TERMINAR" POR MES'
        },
        subtitle: {
            text: ''
        },
        ...
        xAxis: {
            categories: [
			<?php
$sql_re=mysqli_query($conection,"SELECT  count(*) AS ESTADO, DATE_ADD(fecha_pp, INTERVAL 8-DAYOFWEEK(fecha_pp) DAY) SEMANA FROM telar_proceso where falla_pp='Cambio de estilo' AND DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()");
...
?>
        ...
        series: [{
			...
			<?php
$sql_re=mysqli_query($conection,"SELECT  count(*) AS ESTADO, DATE_ADD(fecha_pp, INTERVAL 8-DAYOFWEEK(fecha_pp) DAY) SEMANA FROM telar_proceso where falla_pp='Por terminar' AND DATE_FORMAT(fecha_pp, '%Y-%m-%d') = CURDATE()");
while($res=mysqli_fetch_array($sql_re)){			
?>			
...
<a href="gmts.php" class="btn_new" > Siguiente <span class="fi fi-arrow-right"></a></span></center>
