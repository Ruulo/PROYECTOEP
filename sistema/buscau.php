...
$buscar="";
if($fecha_de > $fecha_a){
	header("local:urdidor1.php");
}else if($fecha_de == $fecha_a){
	
	$where="fecha_u like '$fecha_de%'";
	$buscar="fecha_de=$fecha_de&$fecha_a=$fecha_a";
}else{
	$where="fecha_u BETWEEN '$fecha_de' AND '$fecha_a'";
	$buscar="fecha_de=$fecha_de&$fecha_a=$fecha_a";
      }
   }
?>
...
<!--mostrar datos en tabla-->
          <?php
//paginador 
$sql_registe= mysqli_query($conection,"SELECT COUNT(*) as total FROM urdidor  WHERE $where AND MONTH(fecha_u) = MONTH(CURDATE())");
...
$total_paginas =ceil($total /$por_pagina);
         ...
         $query =mysqli_query($conection,"SELECT * FROM urdidor WHERE $where AND MONTH(fecha_u) = MONTH(CURDATE()) LIMIT $desde, $por_pagina");
